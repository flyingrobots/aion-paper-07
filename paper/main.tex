...
\begin{figure}[H]
	\centering
	% Diagram is forced to full width for print legibility.
	\resizebox{\textwidth}{!}{%
		\begin{tikzpicture}[x=1cm,y=1cm]
			\tikzset{
				warp er node/.style={warp box, minimum width=3.2cm, minimum height=1.05cm, font=\small},
				warp er entity/.style={warp er node},
				warp er result/.style={warp shell, minimum width=3.2cm, minimum height=1.05cm, font=\small},
				warp er rel/.style={draw=black!70, line width=0.8pt},
				warp er label/.style={warp label, font=\small}
			}

			% Pentagonal layout: ring-like ER diagram with curved relations.
			\def\R{4.2}
			\node[warp er entity] (kernel) at (90:\R) {Generic lowering kernel\\\(\operatorname{Lower}_\Psi\)};
			\node[warp er entity] (plans) at (162:\R) {Compiled plans\\\(\rho,\Pi,\Lambda,\Omega\)\\+ host bindings};
			\node[warp er entity] (sdl) at (234:\R) {Authored optic boundary\\(SDL)};
			\node[warp er result] (read) at (306:\R) {Hosted read\\(bounded reveal)};
			\node[warp er result] (shell) at (18:\R) {Retained shell\\\(\theta\) (hologram carrier)};

			% Relations along the ring.
			\draw[warp er rel] (sdl) to[bend left=20]
			node[warp er label, above] {compile}
			(plans);

			\draw[warp er rel] (plans) to[bend left=20]
			node[warp er label, above] {enforce + present}
			(kernel);

			\draw[warp er rel] (kernel) to[bend left=20]
			node[warp er label, above] {pack / retain}
			(shell);

			\draw[warp er rel] (shell) to[bend left=20]
			node[warp er label, below] {serve}
			(read);

			\draw[warp er rel] (read) to[bend left=20]
			node[warp er label, below] {observe}
			(kernel);
		\end{tikzpicture}%
	}
...
\begin{figure}[H]
	...
	\resizebox{\textwidth}{!}{%
		\begin{tikzpicture}[x=1cm,y=1cm]
			\tikzset{
				fish spine/.style={-{Latex[length=3.0mm,width=2.2mm]}, line width=0.95pt, draw=black!75},
				fish bone/.style={line width=0.85pt, draw=black!75},
				fish label/.style={font=\Large, text=black!80},
				fish head/.style={warp box, minimum width=6.2cm, minimum height=1.25cm, font=\Large},
				fish cause/.style={warp box, minimum width=4.8cm, minimum height=1.15cm, font=\Large, fill=black!2},
			}

			% Spine and head (the "problem" we avoid by constraining the seam).
			\coordinate (tail) at (-7.6,0);
			\coordinate (head) at (7.6,0);
			\draw[fish spine] (tail) -- (head);
			\node[fish head, anchor=west] (problem) at (7.65,-0.05) {%
				Unlawful lowering\\across the seam};

			% Bones (causes) with requirement IDs.
			% Upper bones
			\coordinate (u1) at (-4.8,0);
			\draw[fish bone] (u1) -- ++(-2.3,2.0);
			\node[fish cause, anchor=south east] at ($(u1)+(-2.35,2.05)$) {%
				\texttt{REQ-SDL-1}\\
				Non-determinism};

			\coordinate (u2) at (-1.8,0);
			\draw[fish bone] (u2) -- ++(-2.3,2.0);
			\node[fish cause, anchor=south east] at ($(u2)+(-2.35,2.05)$) {%
				\texttt{REQ-COMP-1}\\
				Unchecked boundary};

			\coordinate (u3) at (1.2,0);
			\draw[fish bone] (u3) -- ++(-2.3,2.0);
			\node[fish cause, anchor=south east] at ($(u3)+(-2.35,2.05)$) {%
				\texttt{REQ-READ-1}\\
				Unbounded reveal};

			% Lower bones
			\coordinate (d1) at (-3.3,0);
			\draw[fish bone] (d1) -- ++(-2.3,-2.0);
			\node[fish cause, anchor=north east] at ($(d1)+(-2.35,-2.05)$) {%
				\texttt{REQ-SDL-2}\\
				Unbounded footprint};

			\coordinate (d2) at (-0.3,0);
			\draw[fish bone] (d2) -- ++(-2.3,-2.0);
			\node[fish cause, anchor=north east] at ($(d2)+(-2.35,-2.05)$) {%
				\texttt{REQ-RT-1}\\
				No runtime gate};

			\coordinate (d3) at (2.7,0);
			\draw[fish bone] (d3) -- ++(-2.3,-2.0);
			\node[fish cause, anchor=north east] at ($(d3)+(-2.35,-2.05)$) {%
				\texttt{REQ-RT-2}\\
				Missing witness};
		\end{tikzpicture}%
	}
	\caption{A cause-and-effect view of the compiler seam. The boundary is constrained so that admission remains deterministic (\texttt{REQ-SDL-1}), bounded (\texttt{REQ-SDL-2}), checkable (\texttt{REQ-COMP-1}, \texttt{REQ-RT-1}), witness-bearing (\texttt{REQ-RT-2}), and revealable only under rights (\texttt{REQ-READ-1}).}
	\label{fig:seam-reqs}
\end{figure}
...
side effects. GraphQL is also readily versioned, so new nouns, rewrites, and
