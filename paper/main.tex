...
\begin{figure}[H]
	...
	\begin{tikzpicture}[x=1cm,y=1cm, every node/.style={font=\small}]
		% Participants (sequence-style, simplified for print legibility).
		\node[warp stage, minimum width=2.8cm, minimum height=0.95cm] (sdl) at (0,0)
		{authored SDL\\optic boundary};
		\node[warp stage, minimum width=3.0cm, minimum height=0.95cm] (compiler) at (4.2,0)
		{schema compiler\\plans + bindings};
		\node[warp stage, minimum width=3.0cm, minimum height=0.95cm] (runtime) at (8.4,0)
		{generic runtime\\lower / admit / pack};
		\node[warp shell, minimum width=3.0cm, minimum height=0.95cm] (shell) at (12.6,0)
		{retained shell\\hosted read};

		% Lifelines
		\foreach \p in {sdl,compiler,runtime,shell} {
				\draw[black!35, dashed] (\p.south) -- ++(0,-3.2);
			}

		% Phase labels (kept minimal so they don't fight the arrows).
		\node[warp label, anchor=west] at (-1.05,-1.05) {compile-time};
		\node[warp label, anchor=west] at (-1.05,-2.55) {run-time};

		% Messages (spaced so labels don't overlap).
		\draw[warp edge] (sdl) ++(0,-1.05) -- node[warp label, above] {compile schema} (compiler) ++(0,-1.05);
		\draw[warp edge] (compiler) ++(0,-1.55) -- node[warp label, above] {emit intent + observer plans} (runtime) ++(0,-1.55);

		\draw[warp edge] (runtime) ++(0,-2.55) -- node[warp label, above] {retain shell} (shell) ++(0,-2.55);
		\draw[warp edge] (shell) ++(0,-3.05) -- node[warp label, above] {serve hosted read} (runtime) ++(0,-3.05);
	\end{tikzpicture}
