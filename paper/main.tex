...
observer plans, and shared codecs; and the runtime remains generic, admitting
	...
	\begin{figure}[H]
		\centering
			\begin{tikzpicture}[x=1cm,y=1cm]
				\tikzset{
					warp seam/.style={warp box, minimum height=1.1cm, text width=3.0cm, font=\small},
					warp seam shell/.style={warp shell, minimum height=1.1cm, text width=3.0cm, font=\small},
					warp seam narrow/.style={warp box, minimum height=1.1cm, text width=2.4cm, font=\small},
					warp seam wide/.style={warp box, minimum height=1.1cm, text width=3.4cm, font=\small}
				}

				\node[warp seam wide] (sdl) at (0,0) {Authored optic\\boundary\\(SDL)};
				\node[warp seam narrow] (compiler) at (4.0,0) {Schema compiler};

				\node[warp seam] (intent) at (8.2,2.15) {Write-side intents\\+ host bindings};
				\node[warp seam] (plans) at (8.2,-2.15) {Observer plans\\+ codecs};

				\node[warp seam] (kernel) at (12.4,2.15) {Generic lowering\\\(\operatorname{Lower}_\Psi\)};
				\node[warp seam shell] (shell) at (12.0,0) {Retained shell\\\(\theta\)};
				\node[warp seam] (read) at (12.4,-2.15) {Hosted read\\(bounded revelation)};

				\draw[warp edge] (sdl) -- (compiler);
				\draw[warp edge] (compiler) -- (intent);
				\draw[warp edge] (compiler) -- (plans);
				\draw[warp edge] (intent) -- (kernel);
			...
			\draw[warp edge] (shell) -- node[warp label, right] {serve} (read);
				...
				\node[warp label, anchor=south] at (8.2,3.15) {compiler outputs};
				\node[warp label, anchor=south] at (12.4,3.15) {generic runtime};
			\end{tikzpicture}
