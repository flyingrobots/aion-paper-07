...
\begin{figure}[H]
	...
	\begin{tikzpicture}[node distance=8mm and 9mm]
		\tikzset{
			...
			req edge/.style={-{Latex[length=2.2mm,width=1.8mm]}, semithick},
			req trace/.style={req edge, dashed},
			req label/.style={warp label, font=\small}
		}

		\node[req box] (reqDet) {%
			\textbf{«requirement»} \texttt{REQ-SDL-1}\\
			\textbf{Determinism surface.}\\
			The authored boundary must not depend on ambient non-determinism (wallclock, randomness, hidden I/O).};

		\node[req box, below=of reqDet] (reqFoot) {%
			\textbf{«requirement»} \texttt{REQ-SDL-2}\\
			\textbf{Footprint closure.}\\
			Each rewrite declares reads/writes/creates/forbids and the closures needed to construct its bounded slice \(\chi\).};

		\node[req box, right=of reqDet] (reqComp) {%
			\textbf{«requirement»} \texttt{REQ-COMP-1}\\
			\textbf{Compile-time enforcement.}\\
			The compiler rejects boundaries whose declared footprints and closures cannot be checked.};

		\node[req box, below=of reqComp] (reqRt) {%
			\textbf{«requirement»} \texttt{REQ-RT-1}\\
			\textbf{Runtime footprint gate.}\\
			The lowering kernel enforces the declared footprint during admission.};

		\node[req box, below=of reqRt] (reqWit) {%
			\textbf{«requirement»} \texttt{REQ-RT-2}\\
			\textbf{Witness obligation.}\\
			Admitted acts pack a receipt/shell meeting \(\Lambda\)'s replay and audit obligations.};

		\node[req wide, below=11mm of reqWit, xshift=-0.225\textwidth] (reqRead) {%
			\textbf{«requirement»} \texttt{REQ-READ-1}\\
			\textbf{Bounded revelation.}\\
			Hosted reads are mediated by observer plans and rights, not by direct state access.};

		% Relationships (SysML-ish labels; kept sparse for print legibility).
		\draw[req edge] (reqFoot.east) -- node[req label, above] {deriveReqt} (reqComp.west);
		\draw[req edge] (reqFoot.east) -- ++(6mm,0) |- node[req label, right] {satisfy} (reqRt.west);
		\draw[req edge] (reqRt.south) -- node[req label, right] {satisfy} (reqWit.north);
		\draw[req trace] (reqWit.south) -- node[req label, right] {trace} (reqRead.north);
	\end{tikzpicture}
	\caption{A requirement view of the compiler seam: authored boundaries are constrained so that admission remains deterministic, checkable, and witness-bearing.}
	\label{fig:seam-reqs}
\end{figure}
