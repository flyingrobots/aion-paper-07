...
observer plans, and shared codecs; and the runtime remains generic, admitting
	...
	\begin{figure}[H]
		...
		\begin{tikzpicture}[x=1cm,y=1cm]
			...
			\node[warp seam] (intent) at (8.4,2.15) {Write-side intents\\+ host bindings};
			\node[warp seam] (plans) at (8.4,-2.15) {Observer plans\\+ codecs};

			\node[warp seam] (kernel) at (12.8,2.15) {Generic lowering\\\(\operatorname{Lower}_\Psi\)};
			\node[warp seam shell] (shell) at (12.4,0) {Retained shell\\\(\theta\)};
			\node[warp seam] (read) at (12.8,-2.15) {Hosted read\\(bounded revelation)};

			\draw[warp edge] (sdl) -- node[warp label, above] {compile} (compiler);
			\draw[warp edge] (compiler) -- (intent);
			\draw[warp edge] (compiler) -- (plans);
			\draw[warp edge] (intent) -- node[warp label, above] {present / enforce} (kernel);
			...
			\node[warp label, anchor=south] at (8.4,3.15) {compiler outputs};
			\node[warp label, anchor=south] at (12.8,3.15) {generic runtime};
		\end{tikzpicture}
