...
% Clickable cross-references (make the whole "Section 3", "Definition 3.1", etc. a link).
...
(\Cref{def:warp-optic}).
	...
	audit, replica transport, or bounded revelation (\Cref{sec:warp-optic}). The same act recurs at tick,
	braid, and replica scale (\Cref{sec:scales}), and we ground each scale in a worked example
	(\Cref{sec:examples}).
...
its own witness-bearing outputs (\Cref{prop:closure-witness}): receipts, braid shells,
import shells, and certificates are first-class objects
...
second admission law (\Cref{sec:scales} and \Exref{ex:transport-before-import}).
...
also the reason we adopt the profunctor form in \Cref{subsec:optic-form}: the same declared optic should
...
\begin{enumerate}[leftmargin=*]
	...
	\item A clarification of why the optic form matters: the same declared \WARP{} act may lawfully support execution, footprint derivation, witness extraction, debugger explanation, translation analysis, and schema compilation---that is, compilation of app-authored optic surfaces into runtime-checkable intent and observer artefacts---without changing its structural focus.
		...
		\item A recursive hologram result: each such lowering emits a boundary-complete shell for the bounded admitted situation, so downstream replay and counterfactual examination propagate beyond ticks to braid-local, replica-local, and later trust-bearing shells.

		\item A normalisation thesis for distributed causality: network distance relocates the principal burden into transport and normalisation to a comparable frontier.
...
\Cref{sec:nouns} stabilises the noun map. \Cref{sec:warp-optic} gives the core optic definition and
the local laws needed to read it. \Cref{sec:scales} then states the recurrent
...
work needed to present a comparable slice for lawful judgement.
	...
	\label{def:optic-slice}
		...
		hologram later preserves the result slice needed for later replay and audit,
		transport, or bounded revelation. This generalises the spirit of program slicing---isolating what
...
may be executed as a lowering, read as a slice or footprint derivation, used to
extract or validate witness structure, rendered as a debugger explanation,
analysed for conflict or translation cost, or compiled into runtime artefacts.
That reuse is one of the main reasons the paper needs optic language in
addition to reducer language.

\subsection{Residual, Witness, and Hologram}
\label{subsec:residual-witness-hologram}
...
\label{subsec:commit-fold-reveal}
...
\begin{argument}
	...
	transport the principal burden, because the comparable slice must first be
...
required before the optic can lower the weave. Distribution is often treated
...
unchanged. The principal normalisation burden therefore receives a precise address: transport and
...
This sharpens the paper's branching vocabulary. Irreducible plurality need not
be treated as merge failure; under the governing law it may itself be the
lawful lowered result. That contrasts with convergence-first replicated
...
ahead of what the other participant can yet lawfully treat as shared. For the
	...
	\paragraph{Latency and settlement.}
	This distinction becomes especially important once imported first-order history is
...
\label{thm:derived-independent-suffix-confluence}
...
\label{cor:non-independent-suffixes}
...
\WorkedExample{Local Overlap In One Tick}{ex:tick-overlap}
...
\Cref{fig:tick-overlap} shows the overlapping-site geometry.
...
candidate could not lawfully coexist in that tick.
	...
	In optic notation, the local scheduler lowers the candidate rewrite bundle
...
\WorkedExample{A Braid That Remains Plural}{ex:braid-plural}
...
\Cref{fig:braid-plural} shows the braid geometry and the preserved plural outcome.
...
\WorkedExample{Transport Before Import}{ex:transport-before-import}
...
\Cref{fig:transport-before-import} makes the transport-before-import order explicit.
...
The first line localises the principal burden in transport: the remote suffix is
...
witnessed admission. It is formed by exchangeable witness-bearing
...
observer plans, and shared codecs; and the runtime remains generic, admitting
	...
	\Cref{fig:sdl-compiler-runtime} sketches that seam.

	\begin{figure}[H]
		\centering
		\begin{tikzpicture}[x=1cm,y=1cm]
			\tikzset{
				warp seam/.style={warp box, minimum height=1.1cm, text width=3.2cm, font=\small},
				warp seam shell/.style={warp shell, minimum height=1.1cm, text width=3.2cm, font=\small},
				warp seam narrow/.style={warp box, minimum height=1.1cm, text width=2.6cm, font=\small},
				warp seam wide/.style={warp box, minimum height=1.1cm, text width=3.7cm, font=\small}
			}

			\node[warp seam wide] (sdl) at (0,0) {Authored optic\\boundary\\(SDL)};
			\node[warp seam narrow] (compiler) at (4.3,0) {Schema compiler};

			\node[warp seam] (intent) at (8.9,2.15) {Write-side intents\\+ host bindings};
			\node[warp seam] (plans) at (8.9,-2.15) {Observer plans\\+ codecs};

			\node[warp seam] (kernel) at (13.6,2.15) {Generic lowering\\\(\operatorname{Lower}_\Psi\)};
			\node[warp seam shell] (shell) at (13.2,0) {Retained shell\\\(\theta\)};
			\node[warp seam] (read) at (13.6,-2.15) {Hosted read\\(bounded revelation)};

			\draw[warp edge] (sdl) -- node[warp label, pos=0.52, yshift=8pt] {compile} (compiler);
			\draw[warp edge] (compiler) -- (intent);
			\draw[warp edge] (compiler) -- (plans);
			\draw[warp edge] (intent) -- node[warp label, pos=0.54, yshift=9pt] {present / enforce} (kernel);
			\draw[warp edge] (kernel) -- node[warp label, right] {pack / retain} (shell);
			\draw[warp edge] (plans) -- node[warp label, above] {govern} (read);
			\draw[warp edge] (shell) -- node[warp label, right] {serve} (read);

			\node[warp label, anchor=south] at (8.9,3.15) {compiler outputs};
			\node[warp label, anchor=south] at (13.6,3.15) {generic runtime};
		\end{tikzpicture}
		\caption{Authored optic boundaries compile into two runtime-facing artefacts. Write-side intents drive generic lowering. Read-side observer plans govern bounded revelation over the retained shell.}
...
\begin{center}
	...
	\AIONTableStyleOn
	\begin{tabularx}{\textwidth}{@{}>{\raggedright\arraybackslash}p{0.17\textwidth}>{\raggedright\arraybackslash}p{0.23\textwidth}X@{}}
		\toprule
		\AIONTableHead
		Requirement & Prevents & Why the seam needs it \\
		\midrule
		\texttt{REQ-SDL-1}\\deterministic authored boundary
		& Ambient non-determinism
		& The runtime cannot replay, audit, or compare lowerings if authored surfaces may depend on wall-clock reads, random sources, or hidden side effects. \\
		\texttt{REQ-SDL-2}\\bounded footprint
		& Unbounded reachability
		& The seam must expose which nouns and closures are in play so the slice \(\chi\) is finite, checkable, and lawfully lowerable. \\
		\texttt{REQ-COMP-1}\\checked compilation boundary
		& Unchecked surface-to-runtime drift
		& The compiler must verify slot bindings, closure obligations, and forbidden reachability before arbitrary host code runs. \\
		\texttt{REQ-RT-1}\\runtime gate
		& Bypassing admission
		& Runtime enforcement is required so host code cannot mutate state outside the declared optic surface. \\
		\texttt{REQ-RT-2}\\witness-bearing retention
		& Missing receipt or shell evidence
		& Outcomes must produce retained evidence for replay, audit, transport, and later revelation. \\
		\texttt{REQ-READ-1}\\rights-governed revelation
		& Unbounded reveal
		& Observer plans and authority checks must bound what a hosted read may lawfully emit from a retained shell. \\
		\bottomrule
	\end{tabularx}
	\AIONTableStyleOff

	\smallskip
	\small\emph{Compiler seam requirements at the authored-boundary/runtime interface.}
\end{center}
...
schema. Although this boundary is written in GraphQL SDL, it does not posit
a canonical graph object beneath the editor; it serves instead as a convenient
authoring syntax for an optic boundary: a typed aperture over one hot layer and
its admissible rewrite surfaces.

GraphQL was selected here for several reasons. It provides one versionable
schema in which application authors can declare the typed nouns of a hot
layer---node kinds, edge relations, attachments, and the admissible rewrite
surfaces over them as mutations. That in turn allows a schema
compiler---in the current toolchain---to check footprint obligations, slot
bindings, closure requirements, and forbidden reachability at compile time,
before host code runs. Undeclared footprint widening therefore becomes a
compile-time error rather than a runtime surprise: a rewrite surface cannot
claim one optic slice while depending on a larger one.
The same boundary discipline also excludes a large class of ambient
non-determinism. At this surface the developer is declaring shapes, closures,
and mutations rather than writing arbitrary host code with access to random
sources, wall-clock reads, or hidden side effects. Because the host bindings
are generated from the authored surface, they can in turn be checked for
integrity against that surface both as compiled artefacts and when the runtime
presents or enforces them. GraphQL is also readily versioned, so new nouns,
rewrites, and migrations can be staged as explicit schema evolution, avoiding
...
The central rewrite in that family is \texttt{replaceRangeAsTick}, shown in
\Cref{lst:replace-range-as-tick}, because it
...
generated host bindings that enforce those declarations and remain accountable
to the authored surface. At runtime, the generic lowering kernel uses the
declared closures to construct the bounded slice \(\chi\), applies the
admissibility law \(\Pi\) over the presented claim family, and packs the
declared receipt and retention obligations under \(\Lambda\).

\begin{lstlisting}[
	style=aioncode,
	caption={An authored hot-text optic surface (\texttt{replaceRangeAsTick}) with an explicit footprint, slice closures, and forbidden reachability.},
	label={lst:replace-range-as-tick},
	captionpos=b
]
replaceRangeAsTick(
...
worldline. The closures \texttt{ropeRangeClosure} and
...
We omit the full SDL because this excerpt already exposes the authored pattern:
the developer declares the nouns in scope, the closures that build the local
slice, the causal consequences that must be retained, the materialised reading
that may change, and the regions that remain outside the aperture. In ordinary
software a developer might declare methods over mutable structures. In a
\WARP{}-native system, the more faithful statement is that the developer authors
optic boundaries and lets a schema compiler compile those boundaries into
runtime-checkable footprint, witness, and retention obligations.
...
and archived. Revocation is a new witnessed status transition about future
lawful reliance, not deletion. This creates a true reliance domain
...
Continuum is best understood as the protocol-shaped causal medium defined by witnessed suffixes, transport, slicing,
lowering, outcome algebra, and bounded revelation.
Paper VIII develops that consequence directly by treating Continuum as a
protocol of witnessed admission rather than as a privileged runtime.
...
historical folding, and observer revelation are treated as instances of the same
	...
	a hologram, the same downstream uses recur beyond ticks, including replay and
	audit, transport, and counterfactual examination. Every retained slice contains the least sufficient history of its bounded
...
\label{sec:adversarial-transport}
