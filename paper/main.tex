...
The title writes \emph{WARP} in capitals. In the body we typeset the same name as
\WARP{} (small caps) for legibility. No semantic distinction is intended.
...
Figure~\ref{fig:nouns-roles} summarises the nouns and how the optic acts between them.

\begin{figure}[H]
	\centering
	% Slightly taller than strictly necessary: this reduces the tendency of the
	% following non-splittable block to strand a large blank region.
	\begin{tikzpicture}[node distance=10mm and 14mm, every node/.style={font=\small}]
		\node[warp stage, minimum width=3.3cm] (lane) {causal lane\\history carrier};
		\node[warp small, minimum width=3.1cm, below left=of lane] (worldline) {worldline\\admitted lane};
		\node[warp small, minimum width=3.1cm, below right=of lane] (strand) {strand\\speculative lane};

		\node[warp stage, minimum width=3.3cm, below=14mm of $(worldline)!0.5!(strand)$] (braid) {braid\\lane-scale weave};
		\node[warp stage, minimum width=3.3cm, below=of braid] (weave) {weave\\comparison carrier};
		\node[warp stage, minimum width=3.3cm, below=of weave] (optic) {optic\\lowering act};
		\node[warp shell, minimum width=3.3cm, below=of optic] (hologram) {hologram\\retained boundary};
		\node[warp shell, minimum width=3.3cm, below=of hologram] (shell) {shell\\concrete retained carrier};

		\draw[warp edge] (lane) -- (worldline);
		\draw[warp edge] (lane) -- (strand);
		\draw[warp edge] (worldline.south) .. controls +(0,-7mm) and +(-7mm,7mm) .. (braid.west);
		\draw[warp edge] (strand.south) .. controls +(0,-7mm) and +(7mm,7mm) .. (braid.east);
		\draw[warp edge] (braid) -- (weave);
		\draw[warp edge] (weave) -- (optic);
		\draw[warp edge] (optic) -- (hologram);
		\draw[warp edge] (hologram) -- (shell);
	\end{tikzpicture}
	\caption{Nouns and roles. Lanes carry history; worldlines and strands are admitted and speculative lane forms. A braid is the lane-scale special case of a weave. The optic lowers a weave over a bounded site, the hologram names the retained boundary of that act, and the shell is its concrete carrier.}
	\label{fig:nouns-roles}
\end{figure}

% Keep the specification note and terminology summary from being split by a page
% break, without creating large blank regions.
...
separated from imported settlement over already-judged history. A runtime may
	...
	transport remains hard. The statement localises that difficulty in
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[node distance=9mm]
		\node[warp stage, minimum width=2.8cm, minimum height=1.0cm] (sdl) {authored SDL\\optic boundary};
		\node[warp stage, minimum width=3.1cm, minimum height=1.0cm, right=of sdl] (compiler) {schema compiler\\intent and observer plans};
		\node[warp stage, minimum width=3.0cm, minimum height=1.0cm, right=of compiler] (runtime) {generic runtime\\lower / admit / pack};
		\node[warp shell, minimum width=3.0cm, minimum height=1.0cm, right=of runtime] (shell2) {retained shell\\and hosted read};
		\draw[warp edge] (sdl) -- (compiler);
		\draw[warp edge] (compiler) -- (runtime);
		\draw[warp edge] (runtime) -- (shell2);
	\end{tikzpicture}
